...
public class QQMailServiceImpl implements MailService {
    ...
    public Result<?> sendMailToChangeEmail(String mailTo) {
        logger.info("发送QQ变更邮箱验证邮件: mailTo={}", mailTo);
        return sendQQMail(mailTo, Constants.MailType.CHANGE_EMAIL);
    }

    @Override
    public Result<?> sendMailToChangePwd(String mailTo) {
        logger.info("发送QQ修改密码验证邮件: mailTo={}", mailTo);
        return sendQQMail(mailTo, Constants.MailType.CHANGE_PWD);
    }
